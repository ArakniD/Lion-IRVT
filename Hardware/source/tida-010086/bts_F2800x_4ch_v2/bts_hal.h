...
static inline void BTS_ExAdcSendTxFrame(void){
    if(BTS_ExAdcRxflag==1)// data is available in the FIFO to read
    {
        BTS_ExAdcRxflag =0;

        GPIO_writePin(57, 0);// make cs low and send the data
...
static inline void BTS_ExAdcRead(){
    GPIO_writePin(57, 1);// make cs high
    BTS_HAL_ExAdcRxframe(&BTS_ADC1,SPIA_BASE);
//    SPI_clearInterruptStatus(SPIA_BASE, SPI_INT_RXFF );
    BTS_ExAdcRxflag=1; // flag to hold receive is complete
